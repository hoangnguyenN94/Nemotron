...
# Example entrypoint for running Qwen embedding finetune from the Run:ai UI
...
TRAIN_DATA_PATH="${TRAIN_DATA_PATH:-${WORKSPACE_ROOT}/output/embed/qwen_stage1/train_mined.automodel_unrolled.json}"
CHECKPOINT_DIR="${CHECKPOINT_DIR:-${WORKSPACE_ROOT}/output/embed/qwen_stage2_multinode/checkpoints}"
QWEN_BASE_MODEL="${QWEN_BASE_MODEL:-Qwen/Qwen3-Embedding-8B}"
...
echo "[run.sh] base model: ${QWEN_BASE_MODEL}"
...
exec run-with-env qwen torchrun \
