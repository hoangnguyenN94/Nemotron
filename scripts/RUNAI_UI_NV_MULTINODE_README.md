# Run:ai UI Guide For NV-Embed-v2 Multi-Node Finetuning

This document explains how to run the `nvidia/NV-Embed-v2` embedding finetune workflow on NVIDIA Run:ai by using the web UI only.
...
- the NV Python environment under `/opt/venvs/nv`
- the required training libraries for the validated NV finetune flow
...
- `hf-cache/hub`
- `hf-cache/modules`
- `output/embed/nv_stage1/train_mined.automodel_unrolled.json`
...
cp scripts/runai_ui_nv_finetune.sh /workspace/nemotron_embed/run.sh
...
- `NPROC_PER_NODE=8`
- `NNODES=2`
- `NV_BASE_MODEL=nvidia/NV-Embed-v2`
- `HF_CACHE_ROOT=/workspace/hf-cache`
- `TRAIN_DATA_PATH=/workspace/output/embed/nv_stage1/train_mined.automodel_unrolled.json`
- `CHECKPOINT_DIR=/workspace/output/embed/nv_stage2_multinode/checkpoints`
...
[run.sh] train data: /workspace/output/embed/nv_stage1/train_mined.automodel_unrolled.json
[run.sh] checkpoints: /workspace/output/embed/nv_stage2_multinode/checkpoints
[run.sh] base model: nvidia/NV-Embed-v2
...
No, not for the validated NV flow.
...
```text
Missing training data: /workspace/output/embed/nv_stage1/train_mined.automodel_unrolled.json
```
